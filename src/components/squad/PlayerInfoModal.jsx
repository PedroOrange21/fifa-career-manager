        ...
        <div className={`p-4 bg-well rounded-[24px] border border-border-subtle flex flex-col gap-4 relative ${hideMarketStatus ? 'pb-9' : ''}`}>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-1.5 mt-1"><span className="text-[9px] text-fg-muted font-black uppercase tracking-widest bg-well-strong px-2 py-0.5 rounded">{current.age} Años</span>{!hideMarketStatus && <span className="text-[9px] text-fg-muted font-black uppercase tracking-widest bg-well-strong px-2 py-0.5 rounded">{current.preferredFoot || 'Diestro'}</span>}{(current.marketValue || current.value) ? <span className="text-[9px] text-fg-muted font-black uppercase tracking-widest bg-well-strong px-2 py-0.5 rounded">{abbreviateValue(current.marketValue || current.value)}</span> : null}{current.type === 'Cedido' ? (<span className="text-[8px] flex items-center gap-1 text-yellow-500 font-black uppercase tracking-widest bg-yellow-500/10 px-2 py-0.5 rounded border border-yellow-500/20"><ArrowDownToLine size={10} className="shrink-0" /> Cedido ({formatLoanDuration(current.loanDuration)})</span>) : current.isInitialSquad && !hideMarketStatus ? (<span className="text-[8px] flex items-center gap-1 text-zinc-400 font-black uppercase tracking-wider bg-zinc-500/20 px-2 py-0.5 rounded border border-zinc-500/20"><Shirt size={10} className="shrink-0" /> Plantilla Inicial</span>) : current.type && !(hideMarketStatus && current.type === 'Comprado') ? (<span className={`text-[8px] flex items-center gap-1 px-2 py-0.5 rounded font-black uppercase tracking-wider ${current.type === 'Cantera' ? 'bg-emerald-600/20 text-emerald-400' : 'bg-blue-600/20 text-blue-400'}`}>{current.type === 'Cantera' && <GraduationCap size={10} className="shrink-0" />} {current.type}</span>) : null}</div>
            </div>
          </div>
        </div>
        )}
          ...
          <>
            ...
            {current.type !== 'Cantera' && (
              <>
                ...
                <div className="w-full bg-well rounded-2xl border border-border-subtle divide-y divide-border-subtle overflow-hidden">
                  <DetailRow label="Adquisición" value={current.isInitialSquad ? 'Plantilla Inicial' : current.type} />
                  ...
                  {current.type === 'Comprado' && (
                    <>
                      <DetailRow label="Precio de Compra (€)" value={current.isInitialSquad ? 'No Aplica (Plantilla Inicial)' : formatCurrency(current.value)} />
