...
// de la pantalla, con temporizador visual (ver @keyframes undo-countdown en index.css) tras
...
//
// entry/closing: pendingEndLoan pasa a null en el instante exacto en que se cancela o se
    ...
    >
      <div className="pointer-events-auto bg-black/80 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl overflow-hidden w-full max-w-xs">
        <div className="flex items-center gap-2.5 px-3.5 py-2.5">
          ...
          <p className="flex-1 min-w-0 text-[11px] font-medium text-white/85 truncate">
            Cesión de <span className="font-semibold text-white">{entry.player.name}</span> finalizada
          </p>
          <button type="button" onClick={undoEndLoan} className="shrink-0 text-[10px] font-bold uppercase tracking-wide text-yellow-400 hover:text-yellow-300 active:scale-95 transition-all touch-manipulation">
            Deshacer
          </button>
        </div>
        <div className="h-[3px] bg-white/10">
          {!closing && <div key={entry.player.id + entry.deadline} className="h-full bg-yellow-400" style={{ animation: `undo-countdown ${durationMs}ms linear forwards` }} />}
        </div>
      </div>
    </div>
  );
