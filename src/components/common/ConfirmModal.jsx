...
export default function ConfirmModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/95 z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className={`bg-surface border ${borderClassName} p-6 rounded-[32px] w-full max-w-sm text-center shadow-2xl`}>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onConfirm} className={`flex-1 py-4 rounded-2xl font-black uppercase text-[10px] shadow-lg transition-all ${confirmClassName}`}>{confirmLabel}</button>
        </div>
      </div>
    </div>
  );
